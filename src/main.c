...
int main(int argc, char **argv) {
	...
	for (int i5 = 0; i5 < NUM_TRAINING_SAMPLES; i5++) {
		for (int j = 0; j < NUM_FEATURES; j++) {
			known_points_soa.features[i5*NUM_FEATURES+j] = known_points[i5].features[j];
		}
		known_points_soa.classification_id[i5] = known_points[i5].classification_id;
	}

	Points_SoA new_points_soa;
	for (int i2 = 0; i2 < NUM_TESTING_SAMPLES; i2++) {
		for (int j2 = 0; j2 < NUM_FEATURES; j2++) {
			new_points_soa.features[i2*NUM_FEATURES+j2] = new_points[i2].features[j2];
		}
		new_points_soa.classification_id[i2] = new_points[i2].classification_id;
	}
    ...
    for (int i = 0; i < num_new_points; i++) {
		...
		// 						new_points_soa.classification_id[i], &best_points,
		...
		for (int i3 = 0; i3 < K; i3++) {
			...
			best_points.distance[i3] = MAX_FP_VAL;
			//printf("initialize distance %e\n", bp->distance);
			best_points.classification_id[i3] = (CLASS_ID_TYPE) -1; // unknown
		}
